...
class WorkerError(RuntimeError):
    """Raised when a remediation worker cannot safely execute a stage."""
...
class RemediationWorker:
    ...
    Stage handlers are dependency-injected. The worker never executes customer
    code and never performs repository writes itself; those capabilities remain
    behind separate, security-reviewed adapters.
    """
    ...
    def run_once(self, *, lease_seconds: int = 300) -> WorkerResult | None:
        ...
        try:
            ...
            return WorkerResult(job=failed, completed=False)
    ...
    def _run_stage(self, job: RemediationJob, target: RemediationStatus, handler: StageHandler) -> RemediationJob:
        ...
        if updated.job_id != job.job_id:
            raise WorkerError("stage changed job identity")
        if updated.organization_id != job.organization_id or updated.installation_id != job.installation_id:
            raise WorkerError("stage changed job tenant identity")
        if updated.status != target.value:
            raise WorkerError("stage handler changed workflow status unexpectedly")
